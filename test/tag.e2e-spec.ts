...
describe('tag E2E', () => {
  ...
  it('GET /tag?page={{page}}&limit={{limit}} - Get all', async () => {
    const res = await request(app.getHttpServer())
      .get(`/tag?page={{page}}&limit={{limit}}`)
      .set('Authorization', `Bearer ${token}`)
      .expect(200);
    // you can add more assertions here on res.body
  });

  it('GET /tag/{{id}} - Get one', async () => {
    const res = await request(app.getHttpServer())
      .get(`/tag/{{id}}`)
      .set('Authorization', `Bearer ${token}`)
      .expect(200);
