...
describe('genre E2E', () => {
  ...
  it('DELETE /genre/{{id}} - Delete', async () => {
  ...
  it('GET /genre?page={{page}}&limit={{limit}} - Get all', async () => {
    const res = await request(app.getHttpServer())
      .get(`/genre?page={{page}}&limit={{limit}}`)
      .set('Authorization', `Bearer ${token}`)
      .expect(200);
    // you can add more assertions here on res.body
  });

  it('POST /genre - New', async () => {
    const res = await request(app.getHttpServer())
      .post(`/genre`)
      .set('Authorization', `Bearer ${token}`)
      .send({
  ...
  it('PATCH /genre/{{id}} - Update', async () => {
    const res = await request(app.getHttpServer())
      .patch(`/genre/{{id}}`)
      .set('Authorization', `Bearer ${token}`)
      .send({
  "description": "Gênero atualizado 2"
})
      .expect(200);
    // you can add more assertions here on res.body
  });

  it('GET /genre/{{id}} - Get one', async () => {
    const res = await request(app.getHttpServer())
      .get(`/genre/{{id}}`)
      .set('Authorization', `Bearer ${token}`)
      .expect(200);
