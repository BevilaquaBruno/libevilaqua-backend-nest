...
export class GenreController {
  ...
  async findOne(@Req() req: Request, @Param('id') id: string) {
    ...
    const genre: Genre = await this.genreService.findOne(
      +id,
      reqUser.libraryId,
    );

    if (null == genre)
      throw new HttpException('genre.general.not_found', HttpStatus.NOT_FOUND);
    return genre;
  }
  ...
  async update(
    ...
    const updatedGenre = await this.genreService.update(
      +id,
      updateGenreDto,
      reqUser.libraryId,
    );
