...
export class MailService {
  ...
  async sendUserConfirmation(user: User, token: string, libraryName: string) {
    ...
    const template = readFileSync(this.getTemplatePath(user.language + '/sendUserConfirmation.template.html'), 'utf-8');
    ...
    this.mailerService.sendMail({
      to: user.email,
      subject: process.env['APP_NAME'] + ' - ' + this.i18nService.translate('mail.user_confirmation.confirm_email', { lang: user.language}),
  ...
  async sendResetPasswordRequest(user: User, token: string) {
    ...
    const template = readFileSync(this.getTemplatePath(user.language + '/sendResetPasswordRequest.template.html'), 'utf-8');
    ...
    this.mailerService.sendMail({
      to: user.email,
      subject: process.env['APP_NAME'] + ' - ' + this.i18nService.translate('mail.reset_password.reset_your_password', { lang: user.language}),
  ...
  private getTemplatePath(filename: string): string {
    ...
    const basePath = join(process.cwd(), 'src', 'mail', 'templates', 'en', filename);
